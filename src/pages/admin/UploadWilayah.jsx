...
export default function UploadWilayah() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:8000/api/v1/admin/upload-wilayah', formData, {
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-800">Upload Master Wilayah</h1>
        <p className="text-slate-500 mt-2">Unggah file Excel yang berisi daftar SLS (Satuan Lingkungan Setempat) baru.</p>
      </div>
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-xl p-12 bg-slate-50">
          ...
          <p className="text-sm text-slate-500 mb-6">Wajib memiliki kolom: iddesa, kdkec, nmkec, kddesa, nmdesa</p>
        ...
        <div className="mt-6 flex justify-end">
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={20} /> : 'Mulai Injeksi Data'}
          </button>
        </div>
