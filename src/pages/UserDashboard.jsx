...
export default function UserDashboard() {
    ...
    filteredRaw.forEach(item => {
      if (!aggregated[item.email]) {
        aggregated[item.email] = {
          ...
          target_prelist: 0, target: 0, alokator: 0, 
          status_open: 0, status_draft: 0, status_submitted: 0, status_approved: 0, status_rejected: 0,
          detail_assignment: [] 
        };
      }
      aggregated[item.email].target_prelist += (item.target_prelist || 0);
      aggregated[item.email].target += (item.target || 0);
      aggregated[item.email].alokator += (item.alokator || 0); 
      ...
      aggregated[item.email].detail_assignment.push({
        ...
        desa: item.desa,
        target_prelist: item.target_prelist || 0,
        target: item.target,
        alokator: item.alokator || 0,
    ...
    filteredTimeline.forEach(item => {
      if (!groupByDate[item.tanggal]) {
        groupByDate[item.tanggal] = { 
          ...
          TargetPrelist: 0, Target: 0, Alokator: 0, Approved: 0, Submitted: 0, Draft: 0, Rejected: 0, Open: 0 
        };
      }
      groupByDate[item.tanggal].TargetPrelist += (item.target_prelist || 0);
      groupByDate[item.tanggal].Target += (item.target || 0);
      groupByDate[item.tanggal].Alokator += (item.alokator || 0); 
  ...
  const globalStats = useMemo(() => {
    ...
    const alokator = filteredDataDesa.reduce((acc, curr) => acc + (curr.alokator || 0), 0);
    ...
    return { target_prelist, target, alokator, approved, submitted, draft, rejected, open };
  }, [filteredDataDesa]);
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 shadow-lg">
                <div className="flex justify-between items-end mb-2">
                  ...
                  <span className="text-4xl font-black text-teal-400">{progresPrelist}%</span>
                </div>
                <div className="w-full bg-slate-950 h-3 rounded-full overflow-hidden shadow-inner">
                  <div className="bg-teal-500 h-full transition-all duration-700" style={{ width: `${Math.min(progresPrelist, 100)}%` }}></div>
                </div>
              ...
              <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 shadow-lg">
                ...
                <div className="w-full bg-slate-950 h-3 rounded-full overflow-hidden shadow-inner">
                  <div className="bg-blue-500 h-full transition-all duration-700" style={{ width: `${Math.min(progresTarget, 100)}%` }}></div>
                </div>
              ...
              <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 shadow-lg">
                ...
                <div className="w-full bg-slate-950 h-3 rounded-full overflow-hidden shadow-inner">
                  <div className="bg-purple-500 h-full transition-all duration-700" style={{ width: `${Math.min(progresAlokator, 100)}%` }}></div>
                </div>
                <p className="text-right text-xs text-slate-400 mt-2 font-mono tracking-wider">{totalPerolehan.toLocaleString()} / {totalAlokator.toLocaleString()}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
              {[
                ...
                <div key={i} className="bg-slate-900/40 p-5 rounded-xl border border-slate-700/50 flex flex-col justify-between">
                  <div className="flex justify-between items-start mb-4">
                    <p className="text-xs uppercase font-extrabold text-slate-500 tracking-wider">{item.label}</p>
                  ...
                  <div className="space-y-3 mt-auto">
                    <div className="flex items-center gap-3" title="Komparasi thd Target Prelist">
                      ...
                      <div className="relative flex-1 bg-slate-800 h-1.5 rounded-full overflow-hidden">
                        <div className={`${item.bg} absolute top-0 left-0 h-full rounded-full opacity-80`} style={{ width: `${totalPrelist > 0 ? Math.min((item.val/totalPrelist)*100, 100) : 0}%` }}></div>
                      </div>
                    ...
                    <div className="flex items-center gap-3" title="Komparasi thd Target Alokator">
                      ...
                      <div className="relative flex-1 bg-slate-800 h-1.5 rounded-full overflow-hidden">
                        <div className={`${item.bg} absolute top-0 left-0 h-full rounded-full opacity-80`} style={{ width: `${totalAlokator > 0 ? Math.min((item.val/totalAlokator)*100, 100) : 0}%` }}></div>
                      </div>
