...
export default function TabPetugas({ dataPetugas, dataTimeline, onExport, selectedKecamatan, selectedKelurahan }) {
  ...
  useEffect(() => {
    setCurrentPage(1);
    setExpandedRow(null);
  }, [activeSubTab, dataPetugas, viewMode]);
  ...
  const formatLengkapWaktu = (timestampRaw) => {
    ...
    try {
      ...
      return new Intl.DateTimeFormat('id-ID', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false }).format(dateObj);
    } catch (e) { return timestampRaw; }
  };
  ...
  const getChartDataForPML = (pmlEmail) => {
    ...
    const bawahanMap = {}; 
    dataPetugas.filter(p => p.role?.toUpperCase() === 'PCL').forEach(pcl => {
      if (pcl.detail_assignment?.some(d => s_codes_pml.has(d.assignment_code))) bawahanMap[pcl.email] = pcl.nama;
    });
    ...
    dataTimeline.forEach(tl => {
      ...
      if (isMilikBawahan) {
          ...
          if (keyNama) kelompokTanggal[tgl][keyNama] += (tl.status_approved || tl.approved || 0);
      }
    });
  ...
  const StackedTargets = ({ prelist, usaha, alokator }) => (
    <div className="flex flex-col items-center justify-center space-y-1">
      <span className="text-[10px] font-black font-mono text-teal-400 bg-teal-500/10 px-2 py-0.5 rounded border border-teal-500/20 w-16 text-center shadow-sm" title="Target Prelist">{prelist.toLocaleString('id-ID')}</span>
      <span className="text-[10px] font-black font-mono text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded border border-blue-500/20 w-16 text-center shadow-sm" title="Target Assignment">{usaha.toLocaleString('id-ID')}</span>
      <span className="text-[10px] font-black font-mono text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/20 w-16 text-center shadow-sm" title="Target Alokator">{alokator.toLocaleString('id-ID')}</span>
    </div>
  );
      ...
      {viewMode === 'akumulasi' ? (
        <>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-slate-900/60 p-4 rounded-t-xl border border-slate-700/50 gap-4">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 text-xs text-slate-400">
                <span>Tampilkan</span>
                <select value={itemsPerPage} onChange={(e) => { setItemsPerPage(Number(e.target.value)); }} className="bg-slate-950 border border-slate-700 text-slate-200 rounded px-2 py-1 outline-none focus:border-blue-500">
              ...
              {isExportMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-64 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl z-20 overflow-hidden animate-in fade-in slide-in-from-top-2">
                    <div className="p-1.5">
                      <button onClick={() => { handleExportExcelBPS(dataPetugas); setIsExportMenuOpen(false); }} className="w-full text-left px-3 py-2.5 text-xs font-bold text-slate-200 hover:bg-emerald-600/20 hover:text-emerald-400 rounded-lg transition-colors flex items-center gap-3">
                        <div className="p-1.5 bg-emerald-500/20 rounded-md text-emerald-400"><FileSpreadsheet size={14} /></div> Format Bukti Bayar BPS
                      </button>
                      <button onClick={() => { handleExportSLSFiltered(sortedDataPetugas, dataPetugas); setIsExportMenuOpen(false); }} className="w-full text-left px-3 py-2.5 text-xs font-bold text-slate-200 hover:bg-indigo-600/20 hover:text-indigo-400 rounded-lg transition-colors flex items-center gap-3 mt-1">
                        <div className="p-1.5 bg-indigo-500/20 rounded-md text-indigo-400"><BarChart2 size={14} /></div> Format Rekap & Detail SLS
                      </button>
                    </div>
                  </div>
                </>
            ...
            <table className="w-full text-left border-collapse min-w-[1100px]">
              <thead>
                <tr className="border-b border-slate-700 text-slate-400 text-[10px] uppercase font-bold tracking-wider bg-slate-900/80">
                  ...
                  <th className="p-4 text-center w-28">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <div className="flex items-center gap-1.5 text-[9px] bg-slate-950 px-1.5 py-0.5 rounded-md border border-slate-700/80 shadow-inner">
                        <button onClick={() => requestSort('target_prelist')} className="hover:text-teal-300 text-teal-400 flex items-center">P{getSortIcon('target_prelist')}</button>
                        <div className="w-px h-3 bg-slate-700"></div>
                        <button onClick={() => requestSort('target')} className="hover:text-blue-300 text-blue-400 flex items-center">Asg{getSortIcon('target')}</button>
                        <div className="w-px h-3 bg-slate-700"></div>
                        <button onClick={() => requestSort('alokator')} className="hover:text-purple-300 text-purple-400 flex items-center">A{getSortIcon('alokator')}</button>
                      </div>
                    </div>
                  </th>
                  <th className="p-4 text-center text-blue-400 cursor-pointer hover:bg-blue-900/20 transition-colors" onClick={() => requestSort('status_open')}>
                    <div className="flex items-center justify-center">Open {getSortIcon('status_open')}</div>
                  </th>
                  ...
                  <th className="p-4 text-center text-amber-400 cursor-pointer hover:bg-slate-800/50 transition-colors" onClick={() => requestSort('harusDikerjakanPerHari')}>
                    <div className="flex flex-col items-center justify-center gap-1"><span>Tgt/Hari</span>{getSortIcon('harusDikerjakanPerHari')}</div>
                  </th>
                ...
                {currentData.map((item, idx) => {
                  ...
                  return (
                    <React.Fragment key={idx}>
                      <tr onClick={() => toggleRow(item.email)} className={`hover:bg-slate-800/80 transition-colors cursor-pointer ${isExpanded ? 'bg-slate-800/50' : ''}`}>
                        ...
                        <td className="p-4">
                          <div className="font-bold text-white flex items-center gap-2 max-w-[200px] truncate" title={item.nama}>
                            <span className={`shrink-0 w-2 h-2 rounded-full ${isAman ? 'bg-emerald-400' : 'bg-rose-500'}`}></span> {item.nama}
                          </div>
                        ...
                        <td className="p-4 text-center text-slate-500">{isExpanded ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</td>
                      </tr>
                          ...
                          <td colSpan="11" className="p-0">
                            {activeSubTab === 'pcl' && (
                              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 pl-12 pr-4 py-6 animate-in slide-in-from-top-2 duration-300">
                                <div>
                                  <h4 className="text-xs font-bold text-indigo-400 uppercase flex items-center gap-2 mb-3"><Map size={14} /> Detail Penugasan Region (SLS):</h4>
                                    ...
                                    <table className="w-full text-left text-xs">
                                      <thead className="bg-slate-800/80 text-slate-400 font-semibold border-b border-slate-700/50">
                                        <tr>
                                          ...
                                          <th className="p-3 text-center text-blue-400">Open</th>
                                        ...
                                        {item.detail_assignment?.map((assign, i) => {
                                          ...
                                          return(
                                            <tr key={i} className="hover:bg-slate-800/30">
                                              ...
                                              <td className="p-3"><StackedTargets prelist={aPrelist} usaha={aUsaha} alokator={aAlokator} /></td>
                                              <td className="p-3 text-center font-mono text-blue-400 font-bold align-top pt-5">{aOpen}</td>
                                ...
                                <div>
                                  <h4 className="text-xs font-bold text-emerald-400 uppercase flex items-center gap-2 mb-3"><TrendingUp size={14} /> Kecepatan Validasi Harian</h4>
                              ...
                              <div className="flex flex-col gap-6 pl-12 pr-4 py-6 animate-in fade-in slide-in-from-top-2 duration-300">
                                <div className="w-full">
                                  <div className="flex items-center justify-between mb-3">
                                    <h4 className="text-xs font-bold text-purple-400 uppercase tracking-wider flex items-center gap-2"><Users size={14} /> Tim PCL di Bawah Pengawasan</h4>
                                    <span className="text-[10px] bg-purple-500/10 text-purple-400 px-2 py-0.5 rounded border border-purple-500/20 font-mono">{getAnakBuahPML(item.email).length} PCL Aktif</span>
                                  </div>
                                    ...
                                    <table className="w-full text-left text-xs">
                                      ...
                                      <tbody className="divide-y divide-slate-800/50 font-mono text-slate-300">
                                        {getAnakBuahPML(item.email).length === 0 ? (
                                          <tr><td colSpan={8} className="p-6 text-center text-slate-500 font-sans italic">Tidak ada PCL yang terikat dengan wilayah tugas PML ini.</td></tr>
                                        ) : (
                                          getAnakBuahPML(item.email).map((anak, i) => {
                                            ...
                                            return (
                                              <tr key={i} className="hover:bg-slate-800/40 transition-colors">
                                                ...
                                                <td className="p-3"><StackedTargets prelist={anak.target_prelist} usaha={anak.target} alokator={anak.alokator} /></td>
                                                <td className="p-3 text-center font-mono text-blue-400 font-bold align-top pt-5">{anak.open}</td>
                                ...
                                <div className="w-full">
                                  <h4 className="text-xs font-bold text-emerald-400 uppercase tracking-wider flex items-center gap-2 mb-3"><BarChart2 size={14} /> Radar Perolehan Harian Tim (Approved)</h4>
                                  <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-800 h-[280px] w-full">
                                    {getChartDataForPML(item.email).length === 0 ? (
                                      <div className="flex flex-col items-center justify-center h-full text-slate-500 italic text-xs"><TrendingUp size={24} className="mb-2 opacity-20" /><span>Menunggu aktivitas sinkronisasi dari lapangan...</span></div>
                                    ) : (
                                      <ResponsiveContainer width="100%" height="100%">
                                        <LineChart data={getChartDataForPML(item.email)}>
                                          ...
                                          <Tooltip contentStyle={{ backgroundColor: '#020617', borderColor: '#334155', borderRadius: '8px' }} labelStyle={{ color: '#94a3b8', fontWeight: 'bold', fontSize: '11px', marginBottom: '8px' }} itemStyle={{ fontSize: '12px', padding: '2px 0' }}/>
                                          ...
                                          {getAnakBuahPML(item.email).map((anak, index) => (
                                            <Line key={anak.nama} type="monotone" dataKey={anak.nama} stroke={`hsl(${(index * 137) % 360}, 85%, 60%)`} strokeWidth={2.5} dot={{ r: 3, strokeWidth: 1 }} activeDot={{ r: 6, strokeWidth: 0 }}/>
                                          ))}
                                        </LineChart>
                                      </ResponsiveContainer>
                                    )}
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-900/60 p-4 rounded-b-xl border-b border-l border-r border-slate-700/50 mt-2">
            <div className="text-xs text-slate-400 font-medium">Menampilkan <span className="text-white font-bold">{totalItems > 0 ? startIndex + 1 : 0}</span> hingga <span className="text-white font-bold">{endIndex}</span> dari <span className="text-white font-bold">{totalItems}</span> entri</div>
            <div className="flex items-center gap-2">
              <button onClick={() => setCurrentPage(currentPage - 1)} disabled={currentPage === 1} className="flex items-center gap-1 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed text-slate-300 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border border-slate-700"><ChevronLeft size={14} /> Prev</button>
              <div className="text-xs font-bold text-slate-300 bg-slate-950 px-3 py-1.5 rounded-lg border border-slate-800">{currentPage} / {totalPages || 1}</div>
              <button onClick={() => setCurrentPage(currentPage + 1)} disabled={currentPage === totalPages || totalPages === 0} className="flex items-center gap-1 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed text-slate-300 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border border-slate-700">Next <ChevronRight size={14} /></button>
            </div>
          </div>
        </>
      ) : (
